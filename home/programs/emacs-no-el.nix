...
orderless	
    ...
    mkIf cfg.enable {
      ...
      custom.programs.emacs-no-el.finalPackage = (emacsWithPackagesFromUsePackage {
        alwaysEnsure = true;
	...
	config = ""; # not used as defaultInitFile below is non-bool
      ...
      custom.programs.shell.shellAliases = { } // optionalAttrs (isLinux && isAarch64) { emacs-no-el = "emacs-no-el -nw"; };
	...
	;*/
	inherit
        (config.custom.programs.emacs-no-el)
	homePackage
	;
      };
    };

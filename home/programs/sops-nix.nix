{ config, lib, pkgs, inputs, homeDir, ... }:
...
#systemd.user.services.podman-paperless.unitConfig.After = [ "sops-nix.service" ];
    sops = {
      ...
      secrets = {
        "paperless/admin_password" = { key = "paperless/admin_password"; };	
        "paperless/secret_key" = { key = "paperless/secret_key"; };
        "paperless/db_password" = { key = "paperless/db_password"; };

...
{
  lib,
  # TODO regarding `config` here need it for homeDir, which in turn is needed for secrets-sops.yaml
  # but is that the right place to put it ?
      ...
      extraSpecialArgs = {
        ...
        homeDir =
          if isLinux && isAarch64 then
            ...
            config.user.home
          # TODO for now only user nixos anyway on nixos-wsl
          else
